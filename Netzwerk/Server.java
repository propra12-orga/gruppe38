...
public class Server extends Thread {
	...
	public static DatagramPacket packet = new DatagramPacket(new byte[1024],
			1024);
	...
	public void run() {
		...
		try {
			...
			while (Main.runserver == true) {
					...
					if (player1_check == false || player2_check == false) {
						...
						if (player1_check == false) {
							if (data.contains(password)) {
								// Speichere Addresse des ersten Spielers und
								// melde ihn an
						...
						// Spieler2
							...
							if (data.contains(password)) {
								// Speichere Addresse des zweiten Spielers und
								// melde ihn an
				...
				if (startserver == true) {
					// Wenn nun beide Spieler angemeldet sind und das Spiel
					// gestartet ist
					// Frage ankommende Daten ab und fuehre die gewuenschten
					// Aktionen aus
					System.out.println(packet.getAddress());
					if (data.contains("player_up_on")) {
						...
						System.out
								.println("Befehl vom Client runter empfangen");
						if (address1.equals(packet.getAddress())) {
